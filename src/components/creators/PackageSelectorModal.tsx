...
const PackageSelectorModal = ({ isOpen, onClose, onSuccess, lang }: PackageSelectorModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative w-full h-[85vh] md:h-auto md:min-h-[80vh] md:w-[95vw] md:max-w-xl bg-gradient-to-b from-[#0a0a2e] via-[#16213e] to-[#0f0f3d] md:rounded-2xl border border-white/10 flex flex-col">
        {/* Close button */}
        <button
          onClick={onClose}
        ...
        <div className="p-4 md:p-6 flex flex-col h-full">
          ...
          <div className="grid grid-cols-2 gap-2 md:gap-3 flex-1 mb-4">
            {packages.map((pkg) => (
              <button
                key={pkg.id}
              ...
              >
                {/* Popular badge */}
                {pkg.popular && (
                  <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 px-2 py-0.5 bg-gradient-to-r from-pink-500 to-purple-500 rounded-full text-[10px] md:text-xs font-semibold text-white flex items-center gap-1">
                ...
                {/* Trial text */}
                <p className="text-green-400 text-xs md:text-sm font-medium mb-1">
                ...
                <div className={`flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg w-full ${
                  pkg.popular
                    ? 'bg-gradient-to-r from-pink-500 to-purple-500'
                    : 'bg-white/10'
                }`}>
                  {isLoading && selectedPackage === pkg.id ? (
                    <span className="text-white text-xs md:text-sm">{texts.processing[lang]}</span>
                  ) : (
                    <>
