-- Fix and recreate the function with correct types
...
BEGIN
  ...
  FOR v_pool_order IN 1..20 LOOP
    ...
    IF array_length(v_questions_array, 1) >= 300 THEN
      INSERT INTO question_pools (pool_order, questions, questions_en, question_count, version)
      VALUES (
        v_pool_order,
        v_questions_array,  -- This is already jsonb[]
        to_jsonb(v_questions_en_array),  -- Convert to jsonb for questions_en column
        array_length(v_questions_array, 1),
        1
      );
