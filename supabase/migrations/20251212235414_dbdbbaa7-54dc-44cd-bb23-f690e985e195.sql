-- Fix timezone-aware leaderboard cache refresh function
-- Cast text to date for proper comparison
...
BEGIN
  ...
  WITH user_local_dates AS (
    ...
    SELECT 
      ...
      p.country_code,
      p.user_timezone,
      (NOW() AT TIME ZONE COALESCE(p.user_timezone, 'UTC'))::DATE as local_date
